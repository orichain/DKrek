...
void CSHA256_Initialize(CSHA256 *csha256) {
    csha256->bytes = 0;
    csha256->s[0] = 0x6a09e667ul;
    csha256->s[1] = 0xbb67ae85ul;
    csha256->s[2] = 0x3c6ef372ul;
    csha256->s[3] = 0xa54ff53aul;
    csha256->s[4] = 0x510e527ful;
    csha256->s[5] = 0x9b05688cul;
    csha256->s[6] = 0x1f83d9abul;
    csha256->s[7] = 0x5be0cd19ul;
}

void CSHA256_Write(CSHA256 *csha256, const unsigned char* data, size_t len)
{
  const unsigned char* end = data + len;
  size_t bufsize = csha256->bytes % 64;
  if (bufsize && bufsize + len >= 64) {
    // Fill the buffer, and process it.
    memcpy(csha256->buf + bufsize, data, 64 - bufsize);
    csha256->bytes += 64 - bufsize;
    data += 64 - bufsize;
    Transform(csha256->s, csha256->buf);
    bufsize = 0;
  }
  while (end >= data + 64) {
    // Process full chunks directly from the source.
    Transform(csha256->s, data);
    csha256->bytes += 64;
    data += 64;
  }
  if (end > data) {
    // Fill the buffer with what remains.
    memcpy(csha256->buf + bufsize, data, end - data);
    csha256->bytes += end - data;
  }
}

void CSHA256_Finalize(CSHA256 *csha256, unsigned char hash[OUTPUT_SIZE])
{
    unsigned char sizedesc[8];
    WRITEBE64(sizedesc, csha256->bytes << 3);
    CSHA256_Write(csha256, pad, 1 + ((119 - (csha256->bytes % 64)) % 64));
    CSHA256_Write(csha256, sizedesc, 8);
    WRITEBE32(hash, csha256->s[0]);
    WRITEBE32(hash + 4, csha256->s[1]);
    WRITEBE32(hash + 8, csha256->s[2]);
    WRITEBE32(hash + 12, csha256->s[3]);
    WRITEBE32(hash + 16, csha256->s[4]);
    WRITEBE32(hash + 20, csha256->s[5]);
    WRITEBE32(hash + 24, csha256->s[6]);
    WRITEBE32(hash + 28, csha256->s[7]);
}

void sha256(unsigned char *input, size_t length, unsigned char *digest) {

	CSHA256 sha;
    CSHA256_Initialize(&sha);
	CSHA256_Write(&sha, input, length);
	CSHA256_Finalize(&sha, digest);
